...
The file runs and checks for changes in your `original` folder and backs those changes up to your `backup` folder every _ seconds based on an interval you set. By default, the value set is `600` which means your folder is getting backed up every 10 minutes. To change this value, edit the `interval` and give whatever value you want. **Take note** that the value you set is in **seconds**. 
